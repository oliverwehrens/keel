...
Don't leave a bare question. Pose each as **exactly three** viable options as a checklist with
pros/cons, mark the one you recommend `(recommended)`, and leave the boxes empty for the user to
pick with `[x]`. Until they pick, the `(recommended)` option is the default.
