...
Capture the raw intent in `specs/briefing.md`. This is the seed every epic grows from.
Read `specs/README.md` first for conventions.

## When to use

- No `specs/briefing.md` exists yet and the user is describing a new product/initiative.
- A briefing exists and the user wants to revise the problem, goals, or constraints.

## Procedure

1. **Locate the artifact.** If `specs/briefing.md` exists, read it and treat this as a
   refinement. Otherwise copy `specs/_templates/briefing.md` to `specs/briefing.md`.
...
- `specs/briefing.md` has a real Problem, at least one Goal, one Non-goal, and one
  Success metric.
