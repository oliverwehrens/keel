...
- `specs/tech-stack.md` exists and is `accepted`. If not, recommend `sdd-techstack` first.

## Procedure

1. **Draft the architecture.** If `specs/architecture.md` doesn't exist, copy
   `specs/_templates/architecture.md` to `specs/architecture.md` and draft it from the
...
3. **Run the guardian.** Invoke the `architecture-guardian` subagent to review
   `specs/architecture.md` for (a) fidelity to the product spec, (b) internal consistency,
...
- One source of architectural truth: `specs/architecture.md`. Don't scatter decisions across
  files.
