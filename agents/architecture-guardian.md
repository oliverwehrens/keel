...
description: Reviews technical artifacts and implementation against the architecture guidelines, the tech stack, and the product spec, then reports conformance findings. Use during sdd-tech-refine and during implementation to check that work stays faithful to ${CLAUDE_PROJECT_DIR}/specs/architecture.md, ${CLAUDE_PROJECT_DIR}/specs/tech-stack.md, and the product source of truth.
...
1. **Product spec** — `${CLAUDE_PROJECT_DIR}/specs/briefing.md` and the reconciled epics in `${CLAUDE_PROJECT_DIR}/specs/epics/`. This is
   the ultimate source of truth. Technical work serves it and may never redefine it.
2. **Tech stack** — `${CLAUDE_PROJECT_DIR}/specs/tech-stack.md`.
3. **Architecture** — `${CLAUDE_PROJECT_DIR}/specs/architecture.md` (principles, boundaries, key decisions,
   guidelines).
...
- **Consistency & language** — does it use established patterns and the `${CLAUDE_PROJECT_DIR}/specs/glossary.md`
