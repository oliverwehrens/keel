...
what doesn't line up. Read `${CLAUDE_PLUGIN_ROOT}/CONVENTIONS.md` for the status lifecycle. Reconcile assumes
...
Read `${CLAUDE_PROJECT_DIR}/specs/briefing.md`, then verify:
...
Read the other epics in `${CLAUDE_PROJECT_DIR}/specs/epics/`, then verify:
