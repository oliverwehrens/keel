...
2. **Emit inferred artifacts** (only for the chosen scope), each with front-matter
   ...
   - `${CLAUDE_PROJECT_DIR}/specs/architecture.md` — components, boundaries, and cross-cutting patterns as they exist;
     capture significant existing decisions as **retroactive ADRs** (`Status: accepted`).
   - `${CLAUDE_PROJECT_DIR}/specs/glossary.md` — domain terms found in code and tests.
   - `${CLAUDE_PROJECT_DIR}/specs/briefing.md` — an **inferred** problem/users/goals, clearly marked as a guess.
   - `${CLAUDE_PROJECT_DIR}/specs/epics/EPIC-NNN-*.md` — one per existing capability in scope ("as-is" epics), with
